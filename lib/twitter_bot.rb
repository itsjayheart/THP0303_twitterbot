...
#send 5 tweets to journalists
def journalist_tweet(array, client)
  5.times do
    client.update("#{array.sample}, j'ai adoré votre dernier article, c'était très pertinent @the_hacking_pro #bonjour_monde")
  end
end

#like 20
def like_20_most_recent(client)
  client.favorite(client.search("#Bonjour_monde", result_type: "recent").take(20))
end

#follow 20
def follow_20_most_recent(client)
  client.search("#Bonjour_monde", result_type: "recent").take(20).map do |tweet|
    client.follow(tweet.user)
  end
end

#journalist_tweet(handle_list, client)
#like_20_most_recent(client)
follow_20_most_recent(client)
